...
class Gruff::Line < Gruff::Base
  ...
  def draw
    ...
    @norm_data.each_with_index do |data_row, index|
      ...
      if @annotate_final_point.include?(index)
        final_point_arr << {
          :text => data_row[ANNOTATE_OFFSET_INDEX], 
          :x => @last_x, 
          :y => @last_y
        }
      end
  ...
  def annotate_final_point(points)
    ...
    #- TODO print next to last datapoint
    #- TODO put all labels on top.  ie, plot them last
    # TODO add right margin for annotated data point on 31st
    ...
    points.each do |p|
      ...
      annotation = "#{@final_point_label} #{commify(p[:text])}"
